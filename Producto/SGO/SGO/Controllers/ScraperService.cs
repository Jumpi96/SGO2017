...
namespace SGO
{
    public class ScraperService
    {
        ...
        public ScraperService(SGOContext context, Usuario usuario)
        {
            _context = context;
        ...
        private Obra IniciarObra(DataTable hoja)
        {
            ...
            ObrasController contObra = new ObrasController(_context);
        ...
        {
            TipoItemsController contTipoItem = new TipoItemsController(_context);
            var tipoItem = contTipoItem.TipoItemByCaracter(caracterTipoItem);
        ...
        {
            DetalleSubItemsController cont = new DetalleSubItemsController(_context);
            ...
            cont.Insertar(detalleSubItem);
        }

        private SubItemDeItem GetSubItemDeItem(Item item, TipoItem tipoItem, string nombre, string nombreUnidad, double precio)
        {
            SubItemsController contSubItem = new SubItemsController(_context);
            SubItem subItem = contSubItem.SubItemByNombre(nombre);

            if (subItem == null)
            {
                UnidadsController contUnidad = new UnidadsController(_context);
                Unidad unidad = contUnidad.UnidadByNombre(nombreUnidad);
            ...
            SubItemDeItemsController contSubItemDeItem = new SubItemDeItemsController(_context);
            SubItemDeItem subItemDeItem = contSubItemDeItem.SubItemDeItemByItemSubItem(item, subItem);

            if (subItemDeItem == null)
            {
                subItemDeItem = new SubItemDeItem { ID = 0, Item = item, SubItem = subItem };
                subItemDeItem = contSubItemDeItem.Insertar(subItemDeItem);
            }
        ...
        private Rubro GetRubro(DataTable hoja, int i)
        {
            ...
            RubroesController cont = new RubroesController(_context);
            Rubro rubro = cont.RubroByNombre(nombreRubro);
        ...
        private SubRubro GetSubRubro(DataTable hoja, int i, Rubro rubro)
        {
            ...
            SubRubroesController cont = new SubRubroesController(_context);
            SubRubro subRubro = cont.SubrubroByNombreYRubro(nombreSubRubro,rubro);
        ...
        private Item GetItem(DataTable hoja, int i, SubRubro subRubro)
        {
            ...
            ItemsController cont = new ItemsController(_context);
            Item item = cont.ItemByNombreYSubrubro(nombreItem, subRubro);
